...
class ImageResEnhancer:
    ...
    def add_NDVI(self, unpatched_image: np.ndarray) -> np.ndarray:
        '''
        Method to calculate the Normalized Difference Vegetation Index (NDVI) and append it 
        as the 4th band.
        Formula: (NIR - Red) / (NIR + Red)

        Args: 
            unpatched_image (np.ndarray): Final enhanced output (256, 256, 3)-dim from the model 
            after unptaching. 

        Returns: 
            ndvi_output (np.ndarray): NDVI band added (256, 256, 4)-dim output. 
        '''
        red = unpatched_image[:, :, 1].astype(float)
        nir = unpatched_image[:, :, 2].astype(float)
        ...
        ndvi_output = np.concatenate([unpatched_image, ndvi_channel], axis=-1)
        return ndvi_output
...
def main(modelpath: str | Path, inputpath: str | Path, outputpath: str | Path):
    ...
    target_bands = [0, 3, 7] #Green(1), Red(4), NIR(8)
        ...
        for _input_name in files:
            if _input_name.lower().endswith((".tiff", ".tif")):
                ...
                with rasterio.open(input_image_path) as src:
                    image_read = np.random.rand(9, 2048, 2048) #src.read() 
                    image_array = image_read[target_bands, :, :]
                    ...
                    image_to_save = imgEnH.add_NDVI(unpatched_image)
                    imgEnH.imgsave(outputpath, image_to_save, _input_name)
